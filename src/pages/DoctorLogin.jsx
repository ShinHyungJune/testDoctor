...
const DoctorLogin = ({login, history}) => {
	...
	const submit = (event) => {
		...
		axios.post(window.domain + "/api/doctor/login", form)
			.then(response => {
				login({id: form.id, type: "doctor"}, response.data.accessToken, onLogin);
			}).catch(error => {
...
export default connect(null, mapDispatchToProps)(DoctorLogin);